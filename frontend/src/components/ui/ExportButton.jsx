...
// Botão com menu para exportar Excel ou imprimir PDF
// ============================================================
'use client';
  ...
  async function handleExcel() {
    ...
    try {
      ...
      const [kpisR, vendR, cliR, prodR] = await Promise.all([
        fetch(`${API_BASE}/sales/kpis?${params}`).then(r => r.json()),
        fetch(`${API_BASE}/sales/ranking/vendedores?${params}&limit=50`).then(r => r.json()),
        fetch(`${API_BASE}/sales/ranking/clientes?${params}&limit=50`).then(r => r.json()),
        fetch(`${API_BASE}/sales/ranking/produtos?${params}&limit=50`).then(r => r.json()),
      ]);
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
        disabled={loading}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors disabled:opacity-60"
      >
        {loading
